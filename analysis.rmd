...
colnames(success_eval) <- c("LR", "kNN", "SVM", "RT", "Actual")
...
success_eval$kNN <- postseason_test_pred
```
...
knnFit <- train(POSTSEASON_NUM ~ ., data = trup,
                method = "knn",
                trControl=ctrl,
...
# changed tr to val. TN
postseason_predictions <- predict(buckets, tr, method = "anova")
# postseason_predictions <- predict(buckets, val, method = "anova")

# STEP 5: Visualizations!
# changed tr to val. TN
table(postseason_predictions,tr$POSTSEASON_NUM)
# table(postseason_predictions,val$POSTSEASON_NUM)

# STEP 6: Correlation!
# changed tr to val. TN
cor(postseason_predictions,tr$POSTSEASON_NUM)
# cor(postseason_predictions,val$POSTSEASON_NUM)
...
stack_model  <- C5.0(success_train[,-3], success_train[,3])
stack_pred   <- predict(stack_model, success_test)
plot(stack_model)

confusionMatrix(success_eval)

```

# Conclusion

Enhanced oddsmaking and risk management

Bonus calculation for team coaches
